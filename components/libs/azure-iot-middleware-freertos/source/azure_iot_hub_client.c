...
//    configASSERT( ( azureiotmqttGET_PACKET_TYPE( pxIncomingPacket->ucType ) ) == azureiotmqttPACKET_TYPE_SUBACK );
    ...
    if( pxIncomingPacket == NULL )
    	return;
    if(( azureiotmqttGET_PACKET_TYPE( pxIncomingPacket->ucType )) != azureiotmqttPACKET_TYPE_SUBACK )
    	return;
...
//    configASSERT( ( azureiotmqttGET_PACKET_TYPE( pxIncomingPacket->ucType ) ) == azureiotmqttPACKET_TYPE_PUBACK );
    ...
    if( pxIncomingPacket == NULL )
    	return;
    if( ( azureiotmqttGET_PACKET_TYPE( pxIncomingPacket->ucType ) ) != azureiotmqttPACKET_TYPE_PUBACK )
    	return;
...
//            printf("\n *****************************\n");
//            printf("\n xCommandRequest.pvMessagePayload = %s", (char*)xCommandRequest.pvMessagePayload);
//            printf("\nxCommandRequest.pucCommandName = %s", (char*)xCommandRequest.pucCommandName);
//            printf("\n xCommandRequest.ulPayloadLength = %ld", xCommandRequest.ulPayloadLength);
//            printf("\n xCommandRequest.usCommandNameLength = %d", xCommandRequest.usCommandNameLength);
//            printf("\n *****************************\n");
                                         ...
                                         const AzureIoTTransportInterface_t * pxTransportInterface )
{
    ...
    else
    {
        ...
        else
        {
            ...
            pxAzureIoTHubClient->_internal.xTwinCallback =
            pxHubClientOptions == NULL ? NULL : pxHubClientOptions->xTwinCallback;
...
//AzureIoTResult_t AzureIoTHubClient_Deinit(AzureIoTHubClient_t * pxAzureIoTHubClient)
//{
//    AzureIoTResult_t xResult = eAzureIoTSuccess;
//
//    if (pxAzureIoTHubClient == NULL)
//    {
//        AZLogError(("AzureIoTHubClient_Deinit failed: invalid argument"));
//        return eAzureIoTErrorInvalidArgument;
//    }
//
//    /* Clean up Azure IoT MQTT resources */
//    if (AzureIoTMQTT_Disconnect(&(pxAzureIoTHubClient->_internal.xMQTTContext)) != eAzureIoTMQTTSuccess)
//    {
//        AZLogError(("Failed to deinitialize AzureIoTMQTT"));
//        xResult = eAzureIoTErrorFailed;
//    }
//
//    /* Clear working buffer */
//    if (pxAzureIoTHubClient->_internal.pucWorkingBuffer != NULL)
//    {
//        memset(pxAzureIoTHubClient->_internal.pucWorkingBuffer, 0,
//               pxAzureIoTHubClient->_internal.ulWorkingBufferLength);
//        pxAzureIoTHubClient->_internal.pucWorkingBuffer = NULL;
//    }
//
//    /* Clear other internal members */
//    pxAzureIoTHubClient->_internal.pucDeviceID = NULL;
//    pxAzureIoTHubClient->_internal.ulDeviceIDLength = 0;
//    pxAzureIoTHubClient->_internal.pucHostname = NULL;
//    pxAzureIoTHubClient->_internal.ulHostnameLength = 0;
//    pxAzureIoTHubClient->_internal.xTimeFunction = NULL;
//    pxAzureIoTHubClient->_internal.xTelemetryCallback = NULL;
//    pxAzureIoTHubClient->_internal.xTwinCallback = NULL;
//
//    /* Clean up the Azure IoT Hub Client Core if required */
//    // Since az_iot_hub_client_init() doesn't allocate resources, there's no explicit cleanup needed for it.
//
//    AZLogInfo(("AzureIoTHubClient_Deinit completed successfully"));
//    return xResult;
//}
                                            ...
                                            uint32_t ulTimeoutMilliseconds )
{
    ...
    {
       LogError( ( "AzureIoTHubClient_Connect failed: invalid argument" ) );
        xResult = eAzureIoTErrorInvalidArgument;
    }
    else
    {
        ...
        if( az_result_failed( xCoreResult = az_iot_hub_client_get_user_name( &pxAzureIoTHubClient->_internal.xAzureIoTHubClientCore,
                                                                             ...
                                                                             &xMQTTUserNameLength ) ) )
        {
          LogError( ( "Failed to get username: core error=0x%08x", ( uint16_t ) xCoreResult ) );
            xResult = AzureIoT_TranslateCoreError( xCoreResult );
        }
        /* Check if token refresh is set, then generate password */
                 ...
                 ( pxAzureIoTHubClient->_internal.pxTokenRefresh( pxAzureIoTHubClient,
                                                                  ...
                                                                  &ulPasswordLength ) ) )
        {
         LogError( ( "Failed to generate SAS token" ) );
            xResult = eAzureIoTErrorFailed;
        }
        else
        {
            ...
            {
                LogError( ( "Failed to establish MQTT connection: Server=%*s, MQTT error=0x%08x",
                ...
                if(xMQTTResult == eAzureIoTMQTTServerRefused)
                	xResult = eAzureIoTErrorMQTTServerRefused;
                else
                	xResult = eAzureIoTErrorFailed;
            }
            else
            {
                ...
                LogInfo( ( "An MQTT connection is established with %.*s", pxAzureIoTHubClient->_internal.ulHostnameLength,
                             ( const char * ) pxAzureIoTHubClient->_internal.pucHostname ) );
                // console_send_responce_to_console_xface(MSG_STR, "IHUB","Device connected to IOT Hub");
//				 printf("\n *****************************\n");
                                                ...
                                                uint32_t ulTimeoutMilliseconds )
{
                                                       ...
                                                       ulTimeoutMilliseconds ) ) != eAzureIoTMQTTSuccess )
    {
        ...
        printf("\n\n AzureIoTMQTT_ProcessLoop failed: ProcessLoopDuration=%u, MQTT error=0x%08x \n\n",( uint16_t ) ulTimeoutMilliseconds, ( uint16_t ) xMQTTResult);
//        xResult = eAzureIoTErrorFailed;
        if(xMQTTResult == eAzureIoTMQTTKeepAliveTimeout)
        {
        	xResult = eAzureIoTErrorMQTTTimeout;
        }
        else
        {
        	xResult = eAzureIoTErrorFailed;
                                                                  ...
                                                                  uint32_t ulTimeoutMilliseconds)
{
    ...
    if( ( pxAzureIoTHubClient == NULL ) ||
        ( xCallback == NULL ) )
    {
        printf("AzureIoTHubClient_SubscribeCloudToDeviceMessage failed: invalid argument" );
        xResult = eAzureIoTErrorInvalidArgument;
    }
    else
    {
...
//        AZLogDebug( ( "Attempting to subscribe to the MQTT topic: %s", AZ_IOT_HUB_CLIENT_C2D_SUBSCRIBE_TOPIC ) );
        ...
        {
            printf("Cloud to device subscribe failed: MQTT error=0x%08x", xMQTTResult );
        ...
        else
        {
            ...
            {
                printf("Wait for cloud to device sub ack failed : error=0x%08x", xResult);
...
//                      AZ_IOT_HUB_CLIENT_PROPERTIES_MESSAGE_SUBSCRIBE_TOPIC, AZ_IOT_HUB_CLIENT_PROPERTIES_WRITABLE_UPDATES_SUBSCRIBE_TOPIC ) );
                                                      ...
                                                      usSubscribePacketIdentifier ) ) != eAzureIoTMQTTSuccess )
        {
            ...
            if(xMQTTResult == eAzureIoTMQTTBadParameter)
            	xResult = eAzureIoTErrorMQTTBadParameter;
            else
            xResult = eAzureIoTErrorUnsubscribeFailed;
        }
        else
...
//	printf(" done 3 \n");
//	printf("pucReportedPayload = %s \n",(char*)pucReportedPayload );
//	printf("ulReportedPayloadLength = %ld \n",ulReportedPayloadLength );
