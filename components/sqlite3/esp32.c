...
int esp32_Open( sqlite3_vfs * vfs, const char * path, sqlite3_file * file, int flags, int * outflags )
{
	...
	//p->fd = fopen(path, mode);	//spiffs
	//printf("Enter in FS_FOpen function path:%s, mode:%s\n",path, mode);
	FS_FAT_SupportLFN();    // support for long file name

	p->fd = FS_FOpen(path, mode);
    if ( p->fd == NULL ) {
...
int esp32_Close(sqlite3_file *id)
{
	...
	//int rc = fclose(file->fd);	//spiffs
	int rc = FS_FClose(file->fd);
...
int esp32_Read(sqlite3_file *id, void *buffer, int amount, sqlite3_int64 offset)
{
	...
	//ofst = fseek(file->fd, iofst, SEEK_SET);			//spiffs
	 ofst = FS_FSeek(file->fd, iofst, FS_SEEK_SET);
	// printf("Error! FS_seek file [%ld]:[%s].\n", ofst,FS_ErrorNo2Text(ofst));
	if (ofst != 0) {
	...
	//nRead = fread(buffer, 1, amount, file->fd);
	   nRead = FS_FRead((char*)buffer, 1, amount, file->fd);
	if ( nRead == amount ) {
...
int esp32_Write(sqlite3_file *id, const void *buffer, int amount, sqlite3_int64 offset)
{
	...
	//ofst = fseek(file->fd, iofst, SEEK_SET);	//spiffs
	ofst = FS_FSeek(file->fd, iofst, FS_SEEK_SET);
	//printf("iofst:%ld, offset:%lld, amount:%d \n",iofst, offset, amount);
	if (ofst != 0) {
	...
	nWrite = FS_FWrite(buffer, 1, amount, file->fd);  // write string/hex data
	//printf("Buffer:%s, file->fd:%ld \n",(char*)buffer, (long int)file->fd);
	//printf("nWrite:%d, amount:%d \n",nWrite, amount);
	if ( nWrite != amount ) {
...
{
	esp32_file *file = (esp32_file*) id;
	dbg_printf("1 esp32_FileSize: %s: ", file->name);
//	struct stat st;
//	memset(&st, 0, sizeof(struct stat));
//	int rc = stat( file->name, &st );
//	int fno = fileno(file->fd);
//	if (fno == -1)
//	{
//		printf("\n return 1");
...
//	*size = st.st_size;
//	dbg_printf(" %ld[%lld] %ld\n", st.st_size, *size, (long int)file->fd);
//	return SQLITE_OK;
...
////	FS_FILE * pFile 	= 	NULL;
////	//dbg_printf("\n 2 esp32_Access: %s\n", file->name);
////	FS_FOpenEx(file->name, "r", &pFile);		// Open the file in read mode
////
////		if (pFile == NULL) {
////			printf("Error in file open2\n");
////			return (-1);
////		}
		//int rc = FS_GetFileSize(file->fd);

	 *size = FS_GetFileSize(file->fd);
	dbg_printf("\n esp32_FileSize: %s %ld %ld[%lld]\n", file->name,  FS_GetFileSize(file->fd), (long int)file->fd, *size);
	return SQLITE_OK;

//	int fno = fileno(file->fd);
//	if (fno == -1)
//	{
//		dbg_printf("esp32_Access2: %s %d %ld\n", file->name, rc, FS_GetFileSize(file->fd));
//		return SQLITE_IOERR_FSTAT;
//	}
//
////	if (fstat(fno, &st))
...
//	*result = ( rc != -1 );
	...
	int r = FS_GetFileInfo(path, &FileInfo);
//	FS_FILE * pFile 	= 	NULL;
//	dbg_printf("1 esp32_Access: %s\n", path);
//	FS_FOpenEx("test.db", "r", &pFile);		// Open the file in read mode
//		if (pFile == NULL) {
//			printf("Error in file open3\n");
//			//return (-1);
//		}
//		printf("file ptr = %ld", (long int)pFile);

		int rc = FileInfo.FileSize; //FS_GetFileSize(pFile);
			*result = ( rc != 0 );

	dbg_printf("esp32_Access: %s %d %d %ld\n", path, *result, rc,FileInfo.FileSize); //
