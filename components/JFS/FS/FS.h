...
*    of FS_BIGFAT_Open(). More than one flag can be specified
*    and in this case they have to be combined by a bitwise-or
*    operator ('|').
*/
...
*    FS_BIGFAT_Open() and it has to stay valid until the application
...
typedef struct {
  ...
  FS_BIGFAT_OBJ   FileObj;              // Internal. Not to be used by the application.
} FS_BIGFAT_FIND_DATA;
...
*/
int FS_BIGFAT_Close      (FS_BIGFAT_OBJ * pFileObj);
int FS_BIGFAT_GetPos     (FS_BIGFAT_OBJ * pFileObj, U64 * pPos);
int FS_BIGFAT_GetSize    (FS_BIGFAT_OBJ * pFileObj, U64 * pSize);
int FS_BIGFAT_Open       (FS_BIGFAT_OBJ * pFileObj, const char * sFileName, unsigned Flags);
int FS_BIGFAT_Read       (FS_BIGFAT_OBJ * pFileObj,       void * pData, U32 NumBytesToRead, U32 * pNumBytesRead);
int FS_BIGFAT_SetPos     (FS_BIGFAT_OBJ * pFileObj, U64 Pos);
int FS_BIGFAT_SetSize    (FS_BIGFAT_OBJ * pFileObj, U64 Size);
int FS_BIGFAT_Sync       (FS_BIGFAT_OBJ * pFileObj);
int FS_BIGFAT_Write      (FS_BIGFAT_OBJ * pFileObj, const void * pData, U32 NumBytesToWrite, U32 * pNumBytesWritten);
...
*/
typedef void FS_NOR_HW_TYPE_SPI_READ(U8 Unit, U8 * pData, int NumBytes, unsigned int address);
...
*/
typedef void FS_NOR_HW_TYPE_SPI_WRITE(U8 Unit, const U8 * pData, int NumBytes,unsigned int address);
...
*    NOR flash device using FS_NOR_HW_TYPE_SPIFI_READ.
*
...
*    mode byte is located at *(pPara + NumBytesAddr0).
*    The number of dummy bytes to be generated can be calculated as NumBytesPara - NumBytesAddr.
*    One has to be subtracted from the calculate value if FS_NOR_SPI_FLAG_MODE_BYTE is set
*    in Flags. One additional dummy nibble (half-byte) has to be generated if FS_NOR_SPI_FLAG_DUMMY_NIBBLE
*    is set in Flags.
