...
*/
typedef struct {
  int (*pfControl)              (void * pContext, U8 Cmd, unsigned BusWidth);
  int (*pfWrite)                (void * pContext, U8 Cmd, const U8 * pData, unsigned NumBytes, unsigned BusWidth);
  int (*pfRead)                 (void * pContext, U8 Cmd,       U8 * pData, unsigned NumBytes, unsigned BusWidth);
  int (*pfWriteWithAddr)        (void * pContext, U8 Cmd, const U8 * pPara, unsigned NumBytesPara, unsigned NumBytesAddr, const U8 * pData, unsigned NumBytesData, unsigned BusWidth);
  int (*pfReadWithAddr)         (void * pContext, U8 Cmd, const U8 * pPara, unsigned NumBytesPara, unsigned NumBytesAddr,       U8 * pData, unsigned NumBytesData, unsigned BusWidth);
  int (*pfPoll)                 (void * pContext, U8 Cmd, U8 BitPos, U8 BitValue, U32 Delay, U32 TimeOut_ms, unsigned BusWith);
  int (*pfDelay)                (void * pContext, unsigned ms);
  int (*pfControlWithCmdEx)     (void * pContext, const U8 * pCmd, unsigned NumBytesCmd, unsigned BusWidth, unsigned Flags);
  int (*pfWriteWithCmdEx)       (void * pContext, const U8 * pCmd, unsigned NumBytesCmd, const U8 * pData, unsigned NumBytesData, unsigned BusWidth, unsigned Flags);
  int (*pfWriteWithCmdExAndAddr)(void * pContext, const U8 * pCmd, unsigned NumBytesCmd, const U8 * pPara, unsigned NumBytesPara, unsigned NumBytesAddr, const U8 * pData, unsigned NumBytesData, unsigned BusWidth, unsigned Flags);
  int (*pfReadWithCmdExAndAddr) (void * pContext, const U8 * pCmd, unsigned NumBytesCmd, const U8 * pPara, unsigned NumBytesPara, unsigned NumBytesAddr,       U8 * pData, unsigned NumBytesData, unsigned BusWidth, unsigned Flags);
  int (*pfPollWithCmdEx)        (void * pContext, const U8 * pCmd, unsigned NumBytesCmd, const U8 * pPara, unsigned NumBytesPara, unsigned NumBytesAddr, U8 BitPos, U8 BitValue, U32 Delay, U32 TimeOut_ms, unsigned BusWidth, unsigned Flags);
} FS_NOR_SPI_CMD;
...
typedef struct {
  ...
  U16                       BusWidth;                                 // Number of data lines to be used for any other operation except read and write.
  U16                       Flags;                                    // Specifies additional information for the general operations such as if the data has to be transferred on both clock edges.
  U16                       FlagsRead;                                // Specifies additional information for the read memory array operations such as if the data has to be transferred on both clock edges.
  U8                        NumBytesAddr;                             // Number of address bytes (4 bytes for capacity > 128MBit, else 3 bytes)
  U8                        Allow2bitMode;                            // Set to 1 if data can be exchanged via 2 data lines.
  U8                        Allow4bitMode;                            // Set to 1 if data can be exchanged via 4 data lines.
  U8                        AllowOctalMode;                           // Set to 1 if all data can be exchanged via 8 data lines.
  U8                        AllowDTRMode;                             // Set to 1 if all data can be exchanged on both clock edges.
  U8                        CmdRead;                                  // Command to be used for reading the data.
  U8                        CmdReadEx;                                // Command extension to be used for reading the data.
  ...
  U8                        IsDualDeviceMode;                         // Set to 1 if two NOR flash devices are connected in parallel or the data is transferred on both clock edges.
} FS_NOR_SPI_INST;
...
struct FS_NOR_SPI_TYPE {            //lint -esym(9058, FS_NOR_SPI_TYPE) tag unused outside of typedefs. Reason: the typedef is used as forward declaration.
  ...
  void (*pfInit)                    (      FS_NOR_SPI_INST * pInst);
  int  (*pfSetBusWidth)             (      FS_NOR_SPI_INST * pInst);
  int  (*pfSetNumBytesAddr)         (const FS_NOR_SPI_INST * pInst);
  int  (*pfReadApplyPara)           (      FS_NOR_SPI_INST * pInst);
  int  (*pfRemoveWriteProtection)   (const FS_NOR_SPI_INST * pInst, U32 Addr, U32 NumBytes);
  int  (*pfEraseSector)             (const FS_NOR_SPI_INST * pInst, U8 CmdErase, U32 Addr);
  ...
  int  (*pfSetNumCyclesDummy)       (      FS_NOR_SPI_INST * pInst, U32 Freq_Hz);
};
