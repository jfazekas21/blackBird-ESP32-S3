...
File    : FS_NOR_PHY_STM32H743.c
Purpose : NOR physical layer for the internal flash memory of ST STM32H743.
Literature:
...
Additional information:
  ...
  data is written 32 bytes at a time and that the write operations
  are aligned to an 32 byte boundary. In addition, it is not possible
  ...
  bit errors during the low-level format operation of the
...
*/
  ...
  #define FS_NOR_PHY_MAX_NUM_BYTES          4   // Maximum number of bytes that can be changed at once. Permitted values 1, 2 and 4.
#endif
...
*       Misc. registers
*/
#define SYS_FSIZE               (*(volatile U16*)(0x1FF1E880))
...
*       Misc. defines
*/
#define LD_BYTES_PER_SECTOR     17    // Size of a physical sector(power of 2 exponent).
...
*/
  ...
  #define ASSERT_DATA_IS_ALIGNED(Data)                                             \
    if (((Data) & (FS_NOR_LINE_SIZE - 1u)) != 0) {                                 \
      FS_DEBUG_ERROROUT((FS_MTYPE_DRIVER, "NOR_STM32H743: Data is not aligned.")); \
...
*/
  ...
  #define ASSERT_ADDR_IS_ALIGNED(pData)                                               \
    if ((SEGGER_PTR2ADDR(pData) & 3u) != 0) {                                         \
      FS_DEBUG_ERROROUT((FS_MTYPE_DRIVER, "NOR_STM32H743: Address is not aligned.")); \
      FS_X_PANIC(FS_ERRCODE_INVALID_PARA);                                            \
    }
#else
  #define ASSERT_ADDR_IS_ALIGNED(pData)
...
static int _WaitForEndOfOperation(unsigned BankIndex) {
  ...
  if ((Status & (1uL << SR_WRPERR_BIT)) != 0u) {
    FS_DEBUG_ERROROUT((FS_MTYPE_DRIVER, "NOR_STM32H743: Flash operation failed (Flash is write-protected).\n"));
  } else if ((Status & (1uL << SR_PGSERR_BIT)) != 0u) {
    FS_DEBUG_ERROROUT((FS_MTYPE_DRIVER, "NOR_STM32H743: Flash operation failed (Sequence error).\n"));
  } else if ((Status & (1uL << SR_STRBERR_BIT)) != 0u) {
    FS_DEBUG_ERROROUT((FS_MTYPE_DRIVER, "NOR_STM32H743: Flash operation failed (Strobe error).\n"));
  } else if ((Status & (1uL << SR_INCERR_BIT)) != 0u) {
    FS_DEBUG_ERROROUT((FS_MTYPE_DRIVER, "NOR_STM32H743: Flash operation failed (Inconsistency error).\n"));
  } else if ((Status & (1uL << SR_OPERR_BIT)) != 0u) {
    FS_DEBUG_ERROROUT((FS_MTYPE_DRIVER, "NOR_STM32H743: Flash operation failed (Operation error).\n"));
  } else if (TimeOut == 0u) {
    FS_DEBUG_ERROROUT((FS_MTYPE_DRIVER, "NOR_STM32H743: Flash operation failed (Timeout).\n"));
  } else {
    r = 0;
  }
  return r;
...
static void _EnableProgram(unsigned BankIndex) {
  ...
  *pCCR |= 0u
        ...
        | (1uL << SR_CRCEND_BIT)
        ;
  *pCR &= ~(CR_PSIZE_MASK << CR_PSIZE_BIT);
  *pCR |= Parallelism << CR_PSIZE_BIT;
  //
  // Make sure that the data is written to register.
  //
  __ISB();
...
static int _WriteOff(U32 Off, const void * pData, unsigned NumBytes) {
  ...
  //
  // Write the data one flash word (32 bytes) at a time.
  //
  NumItems = NumBytes >> 2;
  for (;;) {
    *pWrite++ = *pData32++;
...
*/
const FS_NOR_PHY_TYPE FS_NOR_PHY_STM32H743 = {
  _PHY_WriteOff,
  _PHY_ReadOff,
  _PHY_EraseSector,
  _PHY_GetSectorInfo,
