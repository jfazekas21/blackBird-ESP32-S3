...
File    : FS_NOR_HW_SPI_RX71M_Renesas_RSKRX71M.c
Purpose : NOR SPI hardware layer for RSKRX71M evaluation board.
Literature:
  [1] RX71M Group User's Manual: Hardware
    (\\FILESERVER\Techinfo\Company\Renesas\MCU\RX\RX71M\RX71M_HardwareManual_Rev1.10.pdf)
  [2] Renesas Starter Kit+ for RX71M CPU Board Schematics
    (\\FILESERVER\Techinfo\Company\Renesas\MCU\RX\EvalBoard\RSK_RX71M\RSK+RX71M_Schema_Rev.1.00.pdf)
  [3] RX71M Group Renesas Starter Kit+ User's Manual
    (\\FILESERVER\Techinfo\Company\Renesas\MCU\RX\EvalBoard\RSK_RX71M\RSK+RX71M_UserManual.pdf)

These are the port pin assignments of the serial NOR flash device on the RSKRX71M evaluation board:

Signal name   Port  Jumper setting    Peripheral
------------------------------------------------
SSLA1-A       PC0   J10, 1-2          GPIO
RSPCKA-B      PC5   J12, 1-2          RSPI
MOSIA-B       PC6   J14, 1-2          RSPI
MISOA-B       PC7   J11, 1-2          RSPI
*/
...
*/
  ...
  #define FS_NOR_HW_SPI_PER_CLOCK_FREQ_KHZ    60000uL   // Frequency of the clock supplied to SPI peripheral.
#endif

#ifndef   FS_NOR_HW_SPI_NOR_CLOCK_FREQ_KHZ
  #define FS_NOR_HW_SPI_NOR_CLOCK_FREQ_KHZ    30000uL   // Frequency of the SPI clock.
#endif

#ifndef   FS_NOR_HW_SPI_USE_OS
  #define FS_NOR_HW_SPI_USE_OS                0         // 0: polling, 1: interrupt
#endif
...
#define PC_PDR            (*(volatile EVENACCESS U8 *)(PC_BASE_ADDR + 0x00))      // Port direction register
#define PC_PODR           (*(volatile EVENACCESS U8 *)(PC_BASE_ADDR + 0x20))      // Port output data register
#define PC_PMR            (*(volatile EVENACCESS U8 *)(PC_BASE_ADDR + 0x60))      // Port mode register
...
#define MPC_PFAOE1        (*(volatile EVENACCESS U8 *)0x0008C105)                 // Address output enable register 1
#define MPC_PC0PFS        (*(volatile EVENACCESS U8 *)0x0008C1A0)                 // Port C pin 0 function select register
#define MPC_PC5PFS        (*(volatile EVENACCESS U8 *)0x0008C1A5)                 // Port C pin 5 function select register
#define MPC_PC6PFS        (*(volatile EVENACCESS U8 *)0x0008C1A6)                 // Port C pin 6 function select register
#define MPC_PC7PFS        (*(volatile EVENACCESS U8 *)0x0008C1A7)                 // Port C pin 7 function select register
...
#define SD_CS_PIN         0   // Chip select         (PC)
#define SD_MOSI_PIN       6   // Master out slave in (PC)
#define SD_CLK_PIN        5   // Clock               (PC)
...
*/
  ...
  __interrupt void                 FS_NOR_HW_ISR_Rx(void);
  __interrupt void                 FS_NOR_HW_ISR_Rx(void) {
#endif
#ifdef __RX
  #pragma interrupt (FS_NOR_HW_ISR_Rx(vect=VECT_SPRI))
  static void                      FS_NOR_HW_ISR_Rx(void) {
#endif
#ifdef __GNUC__
  void __attribute__ ((interrupt)) FS_NOR_HW_ISR_Rx(void);
  void __attribute__ ((interrupt)) FS_NOR_HW_ISR_Rx(void) {
#endif
#if FS_NOR_HW_SPI_USE_OS
...
*/
  ...
  __interrupt void                 FS_NOR_HW_ISR_Tx(void);
  __interrupt void                 FS_NOR_HW_ISR_Tx(void) {
#endif
#ifdef __RX
  #pragma interrupt (FS_NOR_HW_ISR_Tx(vect=VECT_SPTI))
  static void                      FS_NOR_HW_ISR_Tx(void) {
#endif
#ifdef __GNUC__
  void __attribute__ ((interrupt)) FS_NOR_HW_ISR_Tx(void);
  void __attribute__ ((interrupt)) FS_NOR_HW_ISR_Tx(void) {
#endif
#if FS_NOR_HW_SPI_USE_OS
  OS_EnterNestableInterrupt();
...
static void _Read8Bits(U8 * pData) {
  ...
  // Transfer 8 bits.
  //
  RSPI_SPCMD0 &= ~(0xF << SPCMD_SPB);
  RSPI_SPCMD0 |=   0x7 << SPCMD_SPB;
  //
  // Perform 1 transfer.
  //
  RSPI_SPDCR  &= ~0x3;
...
static int _HW_Init(U8 Unit) {
  ...
  MPC_PC0PFS  = 0x00;                   // Controlled by the HW layer
  MPC_PC5PFS  = 0x0D;                   // Controlled by the RSPI peripheral
  MPC_PC6PFS  = 0x0D;                   // Controlled by the RSPI peripheral
  MPC_PC7PFS  = 0x0D;                   // Controlled by the RSPI peripheral
  MPC_PWPR   &= ~(1 << PWPR_PFSWE);     // Restore write protection
  MPC_PWPR   |=   1 << PWPR_B0WI;
  //
  // Make sure that the A16 line which is share with the CS signal is not active.
  //
  MPC_PFAOE1 &= ~(1 << PFAOE1_A16E);
  //
  // Configure the chip select signal as output.
  //
  PC_PODR |= 1 << SD_CS_PIN;            // Active low
  PC_PDR  |= 1 << SD_CS_PIN;
  //
  // MISO is an input signal controlled by SCI.
  //
  PC_PDR &= ~(1 << SD_MISO_PIN);
  PC_PMR |=   1 << SD_MISO_PIN;
  //
  // MOSI is an output signal controlled by SCI.
  //
  PC_PDR |= 1 << SD_MOSI_PIN;
  PC_PMR |= 1 << SD_MOSI_PIN;
  //
  ...
  //
  RSPI_SPSCR  = 0;
  RSPI_SPDCR  = 1 << SPDCR_SPLW;     // 32-bit access to data register
  RSPI_SPCMD0 = 0
              | (1 << SPCMD_CHPA)
              | (1 << SPCMD_CPOL)
...
**********************************************************************
*/
const FS_NOR_HW_TYPE_SPI FS_NOR_HW_SPI_RX71M_Renesas_RSKRX71M = {
  _HW_Init,
  _HW_EnableCS,
  _HW_DisableCS,
  _HW_Read,
