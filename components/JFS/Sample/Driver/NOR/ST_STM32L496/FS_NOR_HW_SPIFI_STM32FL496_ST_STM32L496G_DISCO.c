...
File    : FS_NOR_HW_SPIFI_STM32L496_ST_STM32L496G_DISCO.c
Purpose : Low-level flash driver for STM32L4 QSPI interface.
Literature:
  [1] RM0394 Reference manual STM32L41xxx/42xxx/43xxx/44xxx/45xxx/46xxx advanced Arm-based 32-bit MCUs
    (\\FILESERVER\Techinfo\Company\ST\MCU\STM32\STM32L4\RM0351_Rev6.0_STM32L4x6.pdf)
  [2] Errata sheet STM32L496xx and STM32L4A6xx
    (\\FILESERVER\Techinfo\Company\ST\MCU\STM32\STM32L4\ES0335_Rev13_STM32L496xx_STM32L4A6xx.pdf)
  [3] Datasheet STM32L496xx
    (\\FILESERVER\Techinfo\Company\ST\MCU\STM32\STM32L4\DS_STM32L496xx_Rev4.pdf)
  [4] UM2169 User manual Discovery kit with STM32L496AG MCU
    (\\FILESERVER\Techinfo\Company\ST\MCU\STM32\STM32L4\Evalboard\STM32L496-DISCO\en.DM00353127.pdf)
...
#define RCC_AHB2RSTR            (*(volatile U32*)(RCC_BASE_ADDR + 0x2C))
...
*/
#define AHB2ENR_GPIOAEN         0
#define AHB2ENR_GPIOBEN         1
...
static int _HW_Init(U8 Unit) {
  ...
  GPIOB_AFRH    &= ~(AFR_MASK     << ((NOR_NCS_BIT - 8) << 2));
  GPIOB_AFRH    |=  (0xAuL        << ((NOR_NCS_BIT - 8) << 2));
  ...
  //
  GPIOB_MODER   &= ~(MODER_MASK   << ( NOR_D0_BIT      << 1));
  GPIOB_MODER   |=  (MODER_ALT    << ( NOR_D0_BIT      << 1));
  GPIOB_AFRL    &= ~(AFR_MASK     << ((NOR_D0_BIT - 0) << 2));
  GPIOB_AFRL    |=  (0xAuL        << ((NOR_D0_BIT - 0) << 2));
  GPIOB_OSPEEDR &= ~(OSPEEDR_MASK << ( NOR_D0_BIT      << 1));
  GPIOB_OSPEEDR |=  (OSPEEDR_HIGH << ( NOR_D0_BIT      << 1));
  //
  // D1 is an input/output signal controlled by the QUADSPI unit.
  //
  GPIOB_MODER   &= ~(MODER_MASK   << ( NOR_D1_BIT      << 1));
  GPIOB_MODER   |=  (MODER_ALT    << ( NOR_D1_BIT      << 1));
  GPIOB_AFRL    &= ~(AFR_MASK     << ((NOR_D1_BIT - 0) << 2));
  GPIOB_AFRL    |=  (0xAuL        << ((NOR_D1_BIT - 0) << 2));
  GPIOB_OSPEEDR &= ~(OSPEEDR_MASK << ( NOR_D1_BIT      << 1));
  GPIOB_OSPEEDR |=  (OSPEEDR_HIGH << ( NOR_D1_BIT      << 1));
  //
  // D2 is an input/output signal and is controlled by the QUADSPI unit.
  //
  GPIOA_MODER   &= ~(MODER_MASK   << ( NOR_D2_BIT      << 1));
  GPIOA_MODER   |=  (MODER_ALT    << ( NOR_D2_BIT      << 1));
  GPIOA_AFRL    &= ~(AFR_MASK     << ((NOR_D2_BIT - 0) << 2));
  GPIOA_AFRL    |=  (0xAuL        << ((NOR_D2_BIT - 0) << 2));
  GPIOA_OSPEEDR &= ~(OSPEEDR_MASK << ( NOR_D2_BIT      << 1));
  GPIOA_OSPEEDR |=  (OSPEEDR_HIGH << ( NOR_D2_BIT      << 1));
  //
  // D3 is an output signal and is controlled by the QUADSPI unit.
  //
  GPIOA_MODER   &= ~(MODER_MASK   << ( NOR_D3_BIT      << 1));
  GPIOA_MODER   |=  (MODER_ALT    << ( NOR_D3_BIT      << 1));
  GPIOA_AFRL    &= ~(AFR_MASK     << ((NOR_D3_BIT - 0) << 2));
  GPIOA_AFRL    |=  (0xAuL        << ((NOR_D3_BIT - 0) << 2));
  GPIOA_OSPEEDR &= ~(OSPEEDR_MASK << ( NOR_D3_BIT      << 1));
  GPIOA_OSPEEDR |=  (OSPEEDR_HIGH << ( NOR_D3_BIT      << 1));
#if FS_NOR_HW_USE_OS
  //
...
**********************************************************************
*/
const FS_NOR_HW_TYPE_SPIFI FS_NOR_HW_SPIFI_STM32L496_ST_STM32L496G_DISCO = {
  _HW_Init,
  _HW_SetCmdMode,
  _HW_SetMemMode,
  _HW_ExecCmd,
