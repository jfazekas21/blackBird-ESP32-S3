...
File      : FS_NOR_HW_SPIFI_STM32MP157_ST_STM32MP15x_EV1.c
Purpose   : Low-level flash driver for STM32MP15X QSPI interface.
Literature:
  [1] RM0441 Reference manual STM32MP151 advanced Arm-based 32-bit MPUs
    (\\FILESERVER\Techinfo\Company\ST\MCU\STM32\STM32MP\stm32mp151_RM0441_Rev3.pdf)
  [2] Datasheet STM32MP157C
    (\\FILESERVER\Techinfo\Company\ST\MCU\STM32\STM32MP\stm32mp157c.pdf)
  [3] UM2535 User manual Evaluation boards with STM32MP157 MPUs
    (\\FILESERVER\Techinfo\Company\ST\MCU\STM32\STM32MP\EvalBoard\STM32MP157x-EV1.pdf)
*/
...
**********************************************************************
*/
#ifndef   FS_NOR_HW_NUM_CYCLES_PER_MS
  #define FS_NOR_HW_NUM_CYCLES_PER_MS       10000             // Number of software loops create a 1ms delay
#endif

#ifndef   FS_NOR_HW_PER_CLK_HZ
  #define FS_NOR_HW_PER_CLK_HZ              266000000         // Clock of Quad-SPI unit
#endif

#ifndef   FS_NOR_HW_NOR_CLK_HZ
  #define FS_NOR_HW_NOR_CLK_HZ              (FS_NOR_HW_PER_CLK_HZ / 3)    // Frequency of the clock supplied to NOR flash device
#endif

#ifndef   FS_NOR_HW_MEM_SIZE_SHIFT
  #define FS_NOR_HW_MEM_SIZE_SHIFT          27                // Size of the memory region reserved for QUADSPI (128 MB)
#endif

#ifndef   FS_NOR_HW_USE_OS
  #define FS_NOR_HW_USE_OS                  0                 // Enables/disables the event-driven processing.
#endif
...
*       QSPI registers
*/
#define QUADSPI_BASE_ADDR       0x58003000uL
...
*       Port B registers
*/
#define GPIOB_BASE_ADDR         0x50003000uL
...
*       Port F registers
*/
#define GPIOF_BASE_ADDR         0x50007000uL
...
*/
#define RCC_BASE_ADDR           0x50000000uL
#define RCC_MP_AHB6RSTSETR      (*(volatile U32*)(RCC_BASE_ADDR + 0x198))
#define RCC_MP_AHB6RSTCLRR      (*(volatile U32*)(RCC_BASE_ADDR + 0x19C))
#define RCC_MP_AHB6ENSETR       (*(volatile U32*)(RCC_BASE_ADDR + 0x218))
#define RCC_MP_AHB4ENSETR       (*(volatile U32*)(RCC_BASE_ADDR + 0xA28))

/*********************************************************************
*
*       Masks for the peripheral enable bits
*/
#define AHB4ENR_GPIOBEN_BIT     1
#define AHB4ENR_GPIOFEN_BIT     5
#define AHB6ENR_QSPIEN_BIT      14
...
static int _HW_Init(U8 Unit) {
  ...
  //
  RCC_MP_AHB4ENSETR |= 0
                    | (1uL << AHB4ENR_GPIOBEN_BIT)
                    | (1uL << AHB4ENR_GPIOFEN_BIT)
                    ;
  RCC_MP_AHB6RSTSETR = 1uL << AHB6ENR_QSPIEN_BIT;
  RCC_MP_AHB6RSTCLRR = 1uL << AHB6ENR_QSPIEN_BIT;
  RCC_MP_AHB6ENSETR |= 1uL << AHB6ENR_QSPIEN_BIT;
  //
  // Wait for the unit to exit reset.
  //
  while (1) {
    if ((RCC_MP_AHB6ENSETR & (1uL << AHB6ENR_QSPIEN_BIT)) != 0) {
      break;
    }
  }
  while (1) {
    if ((RCC_MP_AHB4ENSETR & ((1uL << AHB4ENR_GPIOBEN_BIT) | (1uL << AHB4ENR_GPIOFEN_BIT))) != 0) {
      break;
    }
  }
  //
  ...
  //
  BSP_FS_InstallISR(IRQ_INDEX, NULL, 0);
#endif // FS_NOR_HW_USE_OS
  //
  ...
  QUADSPI_CR  = 0
              ...
              | (1uL << CR_SSHIFT_BIT)              // Enable 1/2 cycle shift after the data is driven by the Flash memory.
              | (Div << CR_PRESCALER_BIT)
              | (1uL << CR_APMS_BIT)                // Disable polling mode as soon as a match is detected.
              ;
  QUADSPI_DCR = 0
              ...
              | (FS_NOR_HW_MEM_SIZE_SHIFT << DCR_FSIZE_BIT)    // We set the NOR flash size.
              ;
#if FS_NOR_HW_USE_OS
  //
  ...
  BSP_FS_InstallISR(IRQ_INDEX, _IRQHandler, IRQ_PRIO);
#endif // FS_NOR_HW_USE_OS
  return (int)Freq_Hz;
}
...
*    hardware layer. The time _HW_Delay() blocks does not have to be
...
static int _HW_Delay(U8 Unit, U32 ms) {
  ...
  if (ms != 0u) {
    ...
    do {
      NumCycles = FS_NOR_HW_NUM_CYCLES_PER_MS;
      do {
        ;
      } while (--NumCycles);
    } while (--ms);
...
**********************************************************************
*/
const FS_NOR_HW_TYPE_SPIFI FS_NOR_HW_SPIFI_STM32MP157_ST_STM32MP15x_EV1 = {
  _HW_Init,
  _HW_SetCmdMode,
  _HW_SetMemMode,
  _HW_ExecCmd,
