...
File        : FS_ConfigWinDrive_Documents.c
Purpose     : Configuration file  for Win driver to
              use a folder in the user's Documents folder.
*/
...
void FS_X_AddDevices(void) {
  FILE    * pFile;
  WCHAR     aDocumentFolder[MAX_PATH];
  char      acDocumentFolder[MAX_PATH];
  HRESULT   Result;
  PWSTR     pStr;
  int       Valid;
  //
  ...
  //
  Result = SHGetKnownFolderPath(&FOLDERID_Documents, 0, NULL, &pStr);
  if (Result == S_OK) {
    //
    // Add and configure the driver.
    //
    wcscpy(aDocumentFolder, pStr);
    wcscat(aDocumentFolder, L"\\SEGGER_BASIC_256k_Volume.dat");
    pFile = _wfopen(aDocumentFolder, L"rb");
    if (pFile == NULL) {
      pFile = _wfopen(aDocumentFolder, L"wb");
      if (pFile != NULL) {
    ...
    if (pFile != NULL) {
      fclose(pFile);
      Valid = 1;
      for (unsigned i = 0; i <= wcslen(aDocumentFolder); ++i) {
        acDocumentFolder[i] = (char)aDocumentFolder[i];
        if ((aDocumentFolder[i] & 0xFFFF) > 0x7F) {
          Valid = 0;
        }
      }
      if (Valid) {
        FS_AddDevice(&FS_WINDRIVE_Driver);
        WINDRIVE_Configure(0, acDocumentFolder);
      } else {
        MessageBox(NULL, "The user's 'My Documents' folder path contains non-ASCII characters!", "Can't create disk image", MB_ICONERROR | MB_OK);
      }
    }
  }
  //
