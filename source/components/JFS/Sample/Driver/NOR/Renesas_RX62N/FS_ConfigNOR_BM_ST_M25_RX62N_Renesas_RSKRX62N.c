...
File    : FS_ConfigNOR_BM_ST_M25_RX62N_Renesas_RSKRX62N.c
Purpose : Configuration file for SPI NOR flash devices.
*/
...
*/
#include "FS.h"
#include "FS_NOR_HW_SPI_RX62N_Renesas_RSKRX62N.h"
...
*/
#ifndef   ALLOC_SIZE
  #define ALLOC_SIZE          0x1000        // Size of memory dedicated to the file system.
  ...
  #define NOR_START_ADDR      0x00000000    // Start address of the first sector be used as storage.
                                            // If the entire NOR flash device is used for file system,
                                            // it is identical to the base address.
#endif

#ifndef   NOR_SIZE
  #define NOR_SIZE            0x04000000    // Number of bytes to be used for storage
#endif
...
void FS_X_AddDevices(void) {
  ...
  //
  // Configure the NOR physical layer. The Microchip SST25VF016B NOR flash device
  // mounted on the board does not return a device id that is recognized by the
  // ST M25 NOR physical layer therefore we have to configure it manually here.
  //
  FS_NOR_SPI_SetHWType(0, &FS_NOR_HW_SPI_RX62N_Renesas_RSKRX62N);
  FS_NOR_SPI_Configure(0, 64 * 1024, 32);
  FS_NOR_SPI_SetPageSize(0, 1);
}
