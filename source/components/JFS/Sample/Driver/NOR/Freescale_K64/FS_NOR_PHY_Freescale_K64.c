...
File    : FS_NOR_PHY_Freescale_K64.c
Purpose : Low level flash driver for the internal flash of Freescale Kinetis K64 MCU.
*/
...
#define SIM_SDID                  (*(volatile U32 *)(SIM_BASE_ADDR + 0x24))
...
**********************************************************************
*/
static const NOR_K64_DEVICE _aDevice[] =  {
  {2, 3, 4 * 1024,  1024 * 1024}
};
...
*     Initializes the flash memory controller. It is not allowed to access the flash while accessing the registers.
...
static void _Init(void) {
  ...
  if (_pInst->pDevice == NULL) {
    ...
    FS_DEBUG_LOG((FS_MTYPE_DRIVER, "NOR_K64: Found DeviceID: 0x%02x.\n", DeviceID));
    pDevice    = _aDevice;
    ...
    if (i == NumEntries) {
      FS_DEBUG_ERROROUT((FS_MTYPE_DRIVER, "NOR_K64: Could not identify device."));
      FS_X_PANIC(FS_ERRCODE_UNKNOWN_DEVICE);
    }
  } else {
    FS_DEBUG_LOG((FS_MTYPE_DRIVER, "NOR_K64: Device is selected manually.\n"));
  }
  //
  ...
  if (NumSectors == 0) {
    FS_DEBUG_ERROROUT((FS_MTYPE_DRIVER, "NOR_K64: Flash size to small for this configuration. 0 bytes available."));
  }
...
*
*/
static NOR_K64_INST * _AllocIfRequired(void) {
  if (_pInst == NULL) {
     _pInst = (NOR_K64_INST *)FS_AllocZeroed(sizeof(NOR_K64_INST));
  }
  return _pInst;
}
...
static int _ExecOperation(void) {
  ...
  if (Status & (1 << FSTAT_RDCOLERR)) {
    FS_DEBUG_ERROROUT((FS_MTYPE_DRIVER, "NOR_K64: Flash operation failed. Read collision."));
  } else if (Status & (1 << FSTAT_ACCERR)) {
    FS_DEBUG_ERROROUT((FS_MTYPE_DRIVER, "NOR_K64: Flash operation failed. Invalid command."));
  } else if (Status & (1 << FSTAT_FPVIOL)) {
    FS_DEBUG_ERROROUT((FS_MTYPE_DRIVER, "NOR_K64: Flash operation failed. Flash is write-protected."));
  } else if (Status & (1 << FSTAT_MGSTAT0)) {
    FS_DEBUG_ERROROUT((FS_MTYPE_DRIVER, "NOR_K64: Flash operation failed. Memory controller error."));
  } else {
    r = 0;
  }
  return r;
  ...
  if (_pInst) {
    if (NumBytes) {
      ...
      if (NumItems) {
        do {
          //
          // Re-programming of existing 0s to 0 is not allowed.
          // If FS_NOR_CAN_REWRITE is 0 (which should be configured for this flash)
          // the NOR driver will not overwrite the data in a line
          // and we can skip the write if the data is identical.
          //
          if (FS_MEMCMP((void *)WriteAddr, p, LineSize) == 0) {
            WriteAddr += LineSize;
            p          = (U32 *)((U8 *)p + LineSize);
          } else {
            Data32 = *p++;
...
**********************************************************************
*/
const FS_NOR_PHY_TYPE FS_NOR_PHY_Freescale_K64 = {
  _WriteOff,
  _ReadOff,
  _EraseSector,
  _GetSectorInfo,
