...
----------------------------------------------------------------------
File        : FS_ConfigNOR_BM_K66_SEGGER_emPower.c
Purpose     : FS configuration functions for NOR driver.
---------------------------END-OF-HEADER------------------------------
*/

#include <stdio.h>
#include "FS.h"
#include "FS_NOR_HW_SPI_K66_SEGGER_emPower.h"
...
**********************************************************************
*/
#define ALLOC_SIZE          0x3500        // Size defined in bytes
...
#define FLASH_START_ADDR    0x00000000    // Start address of the first sector be used as storage.
                                          // If the entire chip is used for file system, it is identical to the base address.
#define FLASH_SIZE          0x01000000    // Number of bytes to be used for storage
#define SECTOR_SIZE         512           // Size of the logical sector in bytes
...
void FS_X_AddDevices(void) {
  ...
  FS_NOR_BM_Configure(0, FLASH_START_ADDR, FLASH_START_ADDR, FLASH_SIZE);
  FS_NOR_BM_SetSectorSize(0, SECTOR_SIZE);
#if FS_NOR_SUPPORT_CRC
  FS_NOR_BM_EnableCRC();
  FS_NOR_BM_SetCRCHook(&FS_NOR_CRC_SW);
#endif
  //
  // Configure the NOR physical layer.
  //
  FS_NOR_SFDP_SetHWType(0, &FS_NOR_HW_SPI_K66_SEGGER_emPower);
  ...
  //
  FS_ConfigFileBufferDefault(SECTOR_SIZE, FS_FILE_BUFFER_WRITE);
#endif
}
