...
File    : FS_NOR_HW_SPIFI_STM32H743_ST_STM32H743I_EVAL.c
...
*/
  ...
  #define FS_NOR_HW_SPIFI_PER_CLK_HZ              200000000         // Clock of Quad-SPI unit
#endif

#ifndef   FS_NOR_HW_SPIFI_NOR_CLK_HZ
  #define FS_NOR_HW_SPIFI_NOR_CLK_HZ              25000000          // Frequency of the clock supplied to NOR flash device
#endif

#ifndef   FS_NOR_HW_SPIFI_QUADSPI_MEM_ADDR
  #define FS_NOR_HW_SPIFI_QUADSPI_MEM_ADDR        0x90000000        // This is the start address of the memory region used by the file system
...
#ifndef   FS_NOR_HW_SPIFI_USE_CS2
  #define FS_NOR_HW_SPIFI_USE_CS2                 0                 // Enables or disables the use of the second chip select signal of the QUADSPI peripheral.
                                                                    // On the evaluation board it is not required to enable this signal because the first chip
                                                                    // select signal drives also the second chip select. In addition, it is not allowed to
                                                                    // enable the use of the second chip select signal when using the SDMMC interface because
                                                                    // the same GPIO pin is shared with the data 3 signal of this interface.
#endif
...
static U8 _CalcClockDivider(U32 * pFreq_Hz) {
  ...
  MaxFreq_Hz = *pFreq_Hz;
  while (1) {
    Freq_Hz = FS_NOR_HW_SPIFI_PER_CLK_HZ / (Div + 1);
  ...
  if (SCB_CCR & (1uL << CCR_DC_BIT)) {      // Is data cache enabled?
    ...
    for (iRegion = 0; iRegion < NumRegions; ++iRegion) {
      ...
      Mask = 0uL
           ...
           | (1uL              << RBAR_VALID_BIT)
           ;
      if ((MPU_RBAR & ~Mask) == FS_NOR_HW_SPIFI_QUADSPI_MEM_ADDR) {
        iRegion = NumRegions;
    ...
    if (iRegion < NumRegions) {
      ...
      MPU_RBAR  = FS_NOR_HW_SPIFI_QUADSPI_MEM_ADDR;
      MPU_RASR  = 0
                | (1uL                          << RASR_XN_BIT)
...
static int _HW_Init(U8 Unit) {
  ...
  //
  // D2 of bank 3 is an input/output signal controlled by the QUADSPI unit.
  //
...
**********************************************************************
*/
const FS_NOR_HW_TYPE_SPIFI FS_NOR_HW_SPIFI_STM32H743_ST_STM32H743I_EVAL = {
  _HW_Init,
  _HW_SetCmdMode,
  _HW_SetMemMode,
  _HW_ExecCmd,
