...
*/
  ...
  #define TYPE2NAME(Type) _aDesc[Type]
#endif
...
*    else         Hashcode
*/
static U32 _GetHashCode(const CACHE_RW_QUOTA_DATA * pCacheData, U32 SectorIndex, U8 SectorType) {
  U32 NumSectors;
  ...
  return (SectorIndex % NumSectors) + pCacheData->aHashStart[SectorType];
}
...
*/
static void _Invalidate(CACHE_RW_QUOTA_DATA * pCacheData) {
  U32                         i;
  U32                         NumSectors;
  CACHE_RW_QUOTA_BLOCK_INFO * pBlockInfo;
  U32                         SectorSize;

  NumSectors = pCacheData->NumSectors;
  SectorSize = pCacheData->SectorSize;
  pBlockInfo = SEGGER_PTR2PTR(CACHE_RW_QUOTA_BLOCK_INFO, pCacheData + 1);                                                     // MISRA deviation D:100[d]
  //
  ...
  for (i = 0; i < NumSectors; i++) {
    ...
    pBlockInfo              = SEGGER_PTR2PTR(CACHE_RW_QUOTA_BLOCK_INFO, SEGGER_PTR2PTR(U8, pBlockInfo + 1) + SectorSize);     // MISRA deviation D:100[d]
  }
}
...
*       _ComputeNumSectors
*/
static U32 _ComputeNumSectors(FS_DEVICE * pDevice, CACHE_RW_QUOTA_DATA * pCacheData) {
  U32 NumSectors;
  U16 SectorSize;
  U32 NumBytes;
  ...
  if ((SectorSize == 0u) || (NumBytes < sizeof(CACHE_RW_QUOTA_DATA))) {
    return 0;
  }
  //
  // Compute number of sectors in cache.
  //
  NumSectors = (NumBytes - sizeof(CACHE_RW_QUOTA_DATA)) / (sizeof(CACHE_RW_QUOTA_BLOCK_INFO) + SectorSize);
  if (NumSectors > 0u) {
    pCacheData->NumSectors    = NumSectors;
    pCacheData->SectorSize    = SectorSize;
    _Invalidate(pCacheData);
  }
  return NumSectors;
}
...
*/
static int _CleanBlock(FS_DEVICE * pDevice, const CACHE_RW_QUOTA_BLOCK_INFO * pBlockInfo, U8 SectorType) {
  int r;
  U32 SectorIndex;

  FS_USE_PARA(SectorType);
  SectorIndex = pBlockInfo->SectorIndex;
  FS_DEBUG_LOG((FS_MTYPE_CACHE, "CRWQ: CLEAN VN: \"%s:%d:\", ST: %s, SI: %lu\n", pDevice->pType->pfGetName(pDevice->Data.Unit), pDevice->Data.Unit, TYPE2NAME(SectorType), SectorIndex));
...
*/
static void _UpdateHashAreas(CACHE_RW_QUOTA_DATA * pCacheData) {
  U32      SectorPos;
  U32      SectorPosNext;
  U32      ReqSize;
  unsigned i;
  ...
  for (i = 0; i < FS_SECTOR_TYPE_COUNT; i++) {
    ...
    if (i == (FS_SECTOR_TYPE_COUNT - 1u)) {
      ReqSize = 0xFFFFu;                     // For last type, use the rest of the avail. memory.
    }
...
*/
static void _WriteIntoCache(CACHE_RW_QUOTA_BLOCK_INFO  * pBlockInfo, U32 SectorIndex, const void * pData, U32 SectorSize) {
...
static void _RemoveFromCache(const FS_DEVICE * pDevice, U32 FirstSector, U32 NumSectors) {
  ...
  U32                         iSector;
  CACHE_RW_QUOTA_BLOCK_INFO * pBlockInfo;
  CACHE_RW_QUOTA_DATA       * pCacheData;

  pCacheData = SEGGER_PTR2PTR(CACHE_RW_QUOTA_DATA, pDevice->Data.pCacheData);                                                 // MISRA deviation D:100[d]
  //
  // Iterate over types
  //
  for (iType = 0; iType < 2u; iType++) {                      // Only data and directory caches.
    ...
    if (NumSectorsInCache != 0u) {
      ...
      for (iSector = 0; iSector < NumSectors; iSector++) {
        ...
        Off = HashCode * (sizeof(CACHE_RW_QUOTA_BLOCK_INFO) + pCacheData->SectorSize);
        pBlockInfo = SEGGER_PTR2PTR(CACHE_RW_QUOTA_BLOCK_INFO, SEGGER_PTR2PTR(U8, pCacheData + 1) + Off);                     // MISRA deviation D:100[d]
        if (pBlockInfo->SectorIndex == SectorIndex) {
          FS_DEBUG_LOG((FS_MTYPE_CACHE, "CRWQ: REMOVE VN: \"%s:%d\", SI: %lu\n", pDevice->pType->pfGetName(pDevice->Data.Unit), pDevice->Data.Unit, pBlockInfo->SectorIndex));
          pBlockInfo->SectorIndex = SECTOR_INDEX_INVALID;
          pBlockInfo->IsDirty = 0;
        }
      }
...
static int _Clean(FS_DEVICE * pDevice) {
  ...
  NumSectors = pCacheData->NumSectors;
  SectorSize = pCacheData->SectorSize;
  SizeOfCacheBlock = sizeof(CACHE_RW_QUOTA_BLOCK_INFO) + SectorSize;
  for (i = 0; i < NumSectors; i++) {
    pBlockInfo = SEGGER_PTR2PTR(CACHE_RW_QUOTA_BLOCK_INFO, SEGGER_PTR2PTR(U8, pCacheData + 1) + (i * SizeOfCacheBlock));      // MISRA deviation D:100[d]
    if (pBlockInfo->IsDirty != 0u) {
      Result = _CleanBlock(pDevice, pBlockInfo, FS_SECTOR_TYPE_DATA);
      if (Result != 0) {
        r = Result;
      }
      pBlockInfo->IsDirty = 0;
...
*/
static int _SetMode(FS_DEVICE * pDevice, const CACHE_MODE * pCacheMode) {
  unsigned              i;
  CACHE_RW_QUOTA_DATA * pCacheData;
  U32                   TypeMask;
  int                   r;
...
*/
static int _SetQuota(FS_DEVICE * pDevice, const CACHE_QUOTA * pQuota) {
  unsigned              i;
  CACHE_RW_QUOTA_DATA * pCacheData;
  U32                   TypeMask;
  int                   r;
  ...
  for (i = 0; i < FS_SECTOR_TYPE_COUNT; i++) {
    TypeMask = 1uL << i;
    if ((TypeMask & (U32)pQuota->TypeMask) != 0u) {
...
*    ==0    Sector found.
*/
static int _CacheRWQuota_ReadFromCache(FS_DEVICE * pDevice, U32 SectorIndex, void * pData, U8 SectorType) {
  U32                         Off;
  U32                         SectorSize;
  U32                         HashCode;
  CACHE_RW_QUOTA_DATA       * pCacheData;
  ...
  SectorSize = pCacheData->SectorSize;
  HashCode   = _GetHashCode(pCacheData, SectorIndex, SectorType);
  if (HashCode == 0xFFFFFFFFuL) {
    return 1;                   // Sector not found
  }
  Off         = HashCode * (sizeof(CACHE_RW_QUOTA_BLOCK_INFO) + SectorSize);
  pBlockInfo  = SEGGER_PTR2PTR(CACHE_RW_QUOTA_BLOCK_INFO, SEGGER_PTR2PTR(U8, pCacheData + 1) + Off);                          // MISRA deviation D:100[d]
  if (pBlockInfo->SectorIndex == SectorIndex) {
...
*/
static int _CacheRWQuota_UpdateCache(FS_DEVICE * pDevice, U32 SectorIndex, const void * pData, U8 SectorType) {
  U32                         Off;
  U32                         SectorSize;
  unsigned                    CacheMode;
  CACHE_RW_QUOTA_DATA       * pCacheData;
  ...
  if ((CacheMode & FS_CACHE_MODE_R) != 0u) {      // Read cache is on for this type of sector
    ...
    if ((pBlockInfo->SectorIndex != SectorIndex) && (pBlockInfo->IsDirty != 0u)) {
      r = _CleanBlock(pDevice, pBlockInfo, SectorType);
    }
...
*/
static int _CacheRWQuota_WriteCache(FS_DEVICE * pDevice, U32 SectorIndex, const void * pData, U8 SectorType) {
  U32                         Off;
  U32                         SectorSize;
  U32                         HashCode;
  unsigned                    CacheMode;
...
static void _CacheRWQuota_InvalidateCache(void * p) {
  ...
  pCacheData = SEGGER_PTR2PTR(CACHE_RW_QUOTA_DATA, p);                                                                        // MISRA deviation D:100[d]
  _Invalidate(pCacheData);
...
static int _CacheRWQuota_Command(FS_DEVICE * pDevice, int Cmd, void * p) {
  ...
  case FS_CMD_CACHE_SET_QUOTA:
    r = _SetQuota(pDevice, SEGGER_PTR2PTR(CACHE_QUOTA, p));                                                                   // MISRA deviation D:100[f]
    break;
  ...
  case FS_CMD_CACHE_GET_TYPE:
    {
      ...
      if (pCacheType != NULL) {
        *pCacheType = FS_CacheRWQuota_Init;
        r = 0;
      }
    }
    break;
...
*       _CacheRWQuotaAPI
*/
static const FS_CACHE_API _CacheRWQuotaAPI = {
  _CacheRWQuota_ReadFromCache,
  _CacheRWQuota_UpdateCache,
  _CacheRWQuota_InvalidateCache,
  _CacheRWQuota_Command,
...
U32 FS_CacheRWQuota_Init(FS_DEVICE * pDevice, void * pData, I32 NumBytes) {
  ...
  // Sanity checks.
  //
  FS_DEBUG_ASSERT(FS_MTYPE_API, sizeof(CACHE_RW_QUOTA_DATA)       == FS_SIZEOF_CACHE_RW_QUOTA_DATA);                          //lint !e506 !e774 Constant value Boolean and Boolean within 'if' always evaluates to False N:102.
  FS_DEBUG_ASSERT(FS_MTYPE_API, sizeof(CACHE_RW_QUOTA_BLOCK_INFO) == FS_SIZEOF_CACHE_RW_QUOTA_BLOCK_INFO);                    //lint !e506 !e774 Constant value Boolean and Boolean within 'if' always evaluates to False N:102.
  //
  ...
  //
  if (NumBytesCache < sizeof(CACHE_RW_QUOTA_DATA)) {
    return 0;
  }
  pCacheData = SEGGER_PTR2PTR(CACHE_RW_QUOTA_DATA, pData8);                                                                   // MISRA deviation D:100[d]
  FS_MEMSET(pCacheData, 0, sizeof(CACHE_RW_QUOTA_DATA));
  pDevData->pCacheAPI       = &_CacheRWQuotaAPI;
