...
// Define the twin callback function
void xTwinCallback(
	az_iot_hub_client_twin_response_type response_type,
    az_iot_status response_status,
    az_span payload,
    void* user_context) {

    // Check if the twin update response is successful
    if (response_status == AZ_IOT_STATUS_OK) {
        // Twin update was successful, process the payload
        // You can parse the JSON payload and extract desired and reported properties

        // Example: Print the received payload
        char payload_buffer[512]; // Adjust the buffer size as needed
        az_span_to_str(payload_buffer, sizeof(payload_buffer), payload);
        printf("Received Twin Update: %s\n", payload_buffer);

        // Process the payload further as needed
    } else {
        // Twin update encountered an error, handle the error
        // You can log the error or take appropriate actions
        printf("Twin Update Error: %d\n", response_status);
    }
}
