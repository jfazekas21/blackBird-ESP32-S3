...
//  printf("AZ_JSON_TOKEN_STRING:%s",(char*) AZ_JSON_TOKEN_STRING);
  printf("json_token_kind:%d,AZ_JSON_TOKEN_STRING:%d,AZ_JSON_TOKEN_PROPERTY_NAME:%d,\n",json_token->kind,AZ_JSON_TOKEN_STRING,AZ_JSON_TOKEN_PROPERTY_NAME);
  if (json_token->kind != AZ_JSON_TOKEN_STRING && json_token->kind != AZ_JSON_TOKEN_PROPERTY_NAME)
  {
	 printf("AZ_ERROR_JSON_INVALID_STATE:%d",AZ_ERROR_JSON_INVALID_STATE);
    return AZ_ERROR_JSON_INVALID_STATE;
  }
  ...
  if (!json_token->_internal.string_has_escaped_chars)
  {
    ...
    {
    	printf("AZ_ERROR_NOT_ENOUGH_SPACE:%d",AZ_ERROR_NOT_ENOUGH_SPACE);
      return AZ_ERROR_NOT_ENOUGH_SPACE;
    }
